...
impl VarType {
    pub(super) fn same_var_type(&self, other: &VarType) -> bool {
        match (self, other) {
            ...
            | (VarType::Array(left), VarType::Array(right))
            | (VarType::FunctionReturn(left), VarType::Scalar(right))
            | (VarType::Scalar(left), VarType::FunctionReturn(right))
    ...
    pub(super) fn display_name(&self) -> String {
        match self {
            VarType::Scalar(ty) | VarType::Optional(ty) | VarType::FunctionReturn(ty) => {
                ty.display_name()
            }
