...
- Classic VBA file I/O: file-number I/O is supported for the practical text subset: `FreeFile`, `Open ... For Input|Output|Append|Binary As #n`, `Close`, `Line Input #`, `Input #`, `Print #`, `Write #`, `EOF`, `LOF`, `Seek(n)`, and `Seek #n, position`. File state is owned by the interpreter instance rather than global mutable process state.
- Filesystem helpers: `Dir`, `Kill`, `FileLen`, `CurDir`, `ChDir`, `MkDir`, `RmDir`, and `Name old As new` are available for local filesystem compatibility. `Dir` supports exact paths, `*`/`?` wildcard enumeration, repeated `Dir()` continuation calls, and basic `vbDirectory` matching.
...
6.  **File I/O Scope:** `Open` currently focuses on deterministic local file access. `Random` record I/O, `Access`/`Lock` clauses, record `Len =`, binary `Get`/`Put`, and `FileDateTime` are planned compatibility work, not part of the current safe subset. `ChDir` changes the process working directory, matching classic behavior but remaining process-wide.
